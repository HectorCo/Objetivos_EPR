...
(function() {
  ...
  function renderDashboard() {
    ...
    if (kpiContainer) {
      kpiContainer.innerHTML = `
        ...
        <div class="fb-card">
          ...
          <div class="fb-metric-value">${FB.fmtEUR(t.total)}<span class="fb-metric-suffix">/ ${FB.fmtEUR(state.goals.ingresos)}</span></div>
    ...
    if (escTable) {
      ...
      FB.MONTHS_ESCRITURA.forEach((m, i) => {
        ...
        tbody += `<tr><td style="text-transform:capitalize">${m}</td><td class="num">${FB.fmtEUR(v)}</td></tr>`;
    ...
    if (bonoTable) {
      ...
      trimData.forEach(td => {
        tbody += `<tr>
          ...
          <td class="num">${FB.fmtEUR(td.ingresos)}</td>
  ...
  function renderOpRow(op) {
    ...
    return `<tr>
      ...
      <td class="num">${FB.fmtEUR(op.honorarios)}</td>
      <td style="text-transform:capitalize">${op.escritura}</td>
      <td class="num">${FB.fmtPct(op.pct)}</td>
      <td class="num" style="font-weight:500">${FB.fmtEUR(FB.calcSinIva(op))}</td>
  ...
  function renderOpEditRow(op) {
    ...
    return `<tr class="fb-edit-row">
      ...
      <td class="num" style="color:var(--kimi-color-text-secondary)">${FB.fmtEUR(FB.calcSinIva(op))}</td>
  ...
  function renderResumen() {
    ...
    if (kpiContainer) {
      kpiContainer.innerHTML = `
        <div class="fb-card">
          ...
          <div class="fb-small fb-mt-sm">real: ${FB.fmtEUR(t.total)} · objetivo: ${FB.fmtEUR(state.goals.ingresos)}</div>
        </div>
        <div class="fb-card">
    ...
    if (soldTable) {
      let tbody = state.sold.map((s, i) => `<tr>
        <td>${s.date}</td>
        <td>${s.addr}</td>
        <td class="num">${FB.fmtEUR(s.val)}</td>
    ...
    if (rentTable) {
      let tbody = state.rent.map((r, i) => `<tr>
        <td>${r.date}</td>
        <td>${r.addr}</td>
        <td class="num">${FB.fmtEUR(r.contract)}</td>
        <td class="num">${FB.fmtEUR(r.val)}</td>
        <td class="num" style="width:70px"><span class="fb-row-actions">
  ...
  function renderListas() {
    ...
    if (comTable) {
      let tbody = state.comisiones.map((c, i) => `<tr>
        <td class="num">${FB.fmtEUR(c.sin)}</td>
        <td class="num">${FB.fmtEUR(c.con)}</td>
        <td class="num" style="width:70px"><span class="fb-row-actions">
  ...
  App.newOp = function() {
    ...
    // Mapear a índice del array de 14 meses (nov 2025 = 0, dic 2025 = 1, ene 2026 = 2, ...)
    let monthIndex;
    if (currentYear === 2025) {
      // nov = 10 -> 0, dic = 11 -> 1
      monthIndex = currentMonth - 10;
    } else {
      // 2026 en adelante: ene = 0 -> 2, feb = 1 -> 3, ...
  ...
  App.exportExcel = function() {
    ...
    trimData.forEach(td => { csv += `${td.trimestre}º,${FB.fmtEUR(td.ingresos)},${FB.fmtEUR(td.falta)},${FB.fmt(td.pisos)}\n`; });
    csv += '\n';
