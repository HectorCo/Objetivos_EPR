...
const FBCharts = {
  ...
  renderBarChart(containerId, data, labels, colors) {
    ...
    data.forEach((v, i) => {
      ...
      const displayVal = v > 0 ? FB.fmtEUR(v) : '';
      html += `<div class="fb-bar-group">
        <div class="fb-bar-value">${displayVal}</div>
        <div class="fb-bar" style="height:${h}px;background:${barColor};opacity:${v>0?1:0.15};" title="${label}: ${FB.fmtEUR(v)}"></div>
  ...
  renderLineChart(containerId, actualData, targetData, labels) {
    ...
    for (let i = 0; i <= gridCount; i++) {
      ...
      gridLines += `<text x="${pad.left - 8}" y="${gy + 3}" text-anchor="end" class="axis-text">${FB.fmtEUR(gv)}</text>`;
    }
  ...
  renderDonut(containerId, values, labels, colors) {
    ...
    values.forEach((v, i) => {
      ...
      legend += `<div style="display:flex;align-items:center;gap:6px;font-size:12px;">
        ...
        <span>${labels[i]}: ${FB.fmtEUR(v)}</span>
      </div>`;
